...
void input_bucket_2_new(int i, int sorted_data[dataset_size], int bucket[16][dataset_size/2], int bucket_pointer[16], int start){
	for (int j = start; j < dataset_size/2; j++) {
...
void output_bucket_2_new(int i, int bucket0[2][16][dataset_size/2], int bucket1[2][16][dataset_size/2], int bucket_pointer0[2][16], int bucket_pointer1[2][16]){
	/*
	 * i: ith_radix
	...
	for(l=0; l<8; l++){
		for(m1=0; m1<bucket_pointer0[0][l]; m1++){
			...
			bucket1[0][ith_radix][bucket_pointer1[0][ith_radix]] = bucket0[0][l][m1];
			bucket_pointer1[0][ith_radix] += 1;
		}
		for(m2=0; m2<bucket_pointer0[1][l]; m2++){
			shifted = bucket0[1][l][m2] >> (i * 4);
			ith_radix = shifted & 0xf;
			bucket1[0][ith_radix][bucket_pointer1[0][ith_radix]] = bucket0[1][l][m2];
			bucket_pointer1[0][ith_radix] += 1;
		}
	}
	output_bucket_temp2:
	for(l=8; l<16; l++){
		for(m1=0; m1<bucket_pointer0[0][l]; m1++){
			shifted = bucket0[0][l][m1] >> (i * 4);
			ith_radix = shifted & 0xf;
			bucket1[1][ith_radix][bucket_pointer1[1][ith_radix]] = bucket0[0][l][m1];
			bucket_pointer1[1][ith_radix] += 1;
		}
		for(m2=0; m2<bucket_pointer0[1][l]; m2++){
			shifted = bucket0[1][l][m2] >> (i * 4);
			ith_radix = shifted & 0xf;
			bucket1[1][ith_radix][bucket_pointer1[1][ith_radix]] = bucket0[1][l][m2];
			bucket_pointer1[1][ith_radix] += 1;
		}
	}
...
void radix_sort_separate_bucket_parallel_2_new(int data[dataset_size], int sorted_data[dataset_size]){
	int bucket[2][2][16][dataset_size/2]; // Be careful that dataset_size can't be an odd number.
...
#pragma HLS ARRAY_PARTITION variable=bucket_pointer type=complete dim=1
	...
	for(i=1; i<bit_width_hex; i++){
		output_bucket_2_new(i, bucket[id], bucket[1-id], bucket_pointer[id], bucket_pointer[1-id]);
		id = 1-id;
	}
