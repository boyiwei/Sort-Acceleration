// ==============================================================
// RTL generated by Vitis HLS - High-Level Synthesis from C, C++ and OpenCL v2022.2 (64-bit)
// Version: 2022.2
// Copyright (C) Copyright 1986-2022 Xilinx, Inc. All Rights Reserved.
// 
// ===========================================================

`timescale 1 ns / 1 ps 

module multi_radix_oct_loser_64_radix_sort_oct_batch_39_1_Pipeline_initialization (
        ap_clk,
        ap_rst,
        ap_start,
        ap_done,
        ap_idle,
        ap_ready,
        bucket_sizes_2197_reload,
        bucket_sizes_2196_reload,
        bucket_sizes_2195_reload,
        bucket_sizes_2194_reload,
        bucket_sizes_2193_reload,
        bucket_sizes_2192_reload,
        bucket_sizes_2191_reload,
        bucket_sizes_reload,
        bucket_address0,
        bucket_ce0,
        bucket_we0,
        bucket_d0,
        input_38_address0,
        input_38_ce0,
        input_38_q0,
        bucket_sizes_2213_out_i,
        bucket_sizes_2213_out_o,
        bucket_sizes_2213_out_o_ap_vld,
        bucket_sizes_2212_out_i,
        bucket_sizes_2212_out_o,
        bucket_sizes_2212_out_o_ap_vld,
        bucket_sizes_2211_out_i,
        bucket_sizes_2211_out_o,
        bucket_sizes_2211_out_o_ap_vld,
        bucket_sizes_2210_out_i,
        bucket_sizes_2210_out_o,
        bucket_sizes_2210_out_o_ap_vld,
        bucket_sizes_2209_out_i,
        bucket_sizes_2209_out_o,
        bucket_sizes_2209_out_o_ap_vld,
        bucket_sizes_2208_out_i,
        bucket_sizes_2208_out_o,
        bucket_sizes_2208_out_o_ap_vld,
        bucket_sizes_2207_out_i,
        bucket_sizes_2207_out_o,
        bucket_sizes_2207_out_o_ap_vld,
        bucket_sizes_2206_out_i,
        bucket_sizes_2206_out_o,
        bucket_sizes_2206_out_o_ap_vld
);

parameter    ap_ST_fsm_pp0_stage0 = 1'd1;

input   ap_clk;
input   ap_rst;
input   ap_start;
output   ap_done;
output   ap_idle;
output   ap_ready;
input  [31:0] bucket_sizes_2197_reload;
input  [31:0] bucket_sizes_2196_reload;
input  [31:0] bucket_sizes_2195_reload;
input  [31:0] bucket_sizes_2194_reload;
input  [31:0] bucket_sizes_2193_reload;
input  [31:0] bucket_sizes_2192_reload;
input  [31:0] bucket_sizes_2191_reload;
input  [31:0] bucket_sizes_reload;
output  [18:0] bucket_address0;
output   bucket_ce0;
output   bucket_we0;
output  [31:0] bucket_d0;
output  [17:0] input_38_address0;
output   input_38_ce0;
input  [31:0] input_38_q0;
input  [31:0] bucket_sizes_2213_out_i;
output  [31:0] bucket_sizes_2213_out_o;
output   bucket_sizes_2213_out_o_ap_vld;
input  [31:0] bucket_sizes_2212_out_i;
output  [31:0] bucket_sizes_2212_out_o;
output   bucket_sizes_2212_out_o_ap_vld;
input  [31:0] bucket_sizes_2211_out_i;
output  [31:0] bucket_sizes_2211_out_o;
output   bucket_sizes_2211_out_o_ap_vld;
input  [31:0] bucket_sizes_2210_out_i;
output  [31:0] bucket_sizes_2210_out_o;
output   bucket_sizes_2210_out_o_ap_vld;
input  [31:0] bucket_sizes_2209_out_i;
output  [31:0] bucket_sizes_2209_out_o;
output   bucket_sizes_2209_out_o_ap_vld;
input  [31:0] bucket_sizes_2208_out_i;
output  [31:0] bucket_sizes_2208_out_o;
output   bucket_sizes_2208_out_o_ap_vld;
input  [31:0] bucket_sizes_2207_out_i;
output  [31:0] bucket_sizes_2207_out_o;
output   bucket_sizes_2207_out_o_ap_vld;
input  [31:0] bucket_sizes_2206_out_i;
output  [31:0] bucket_sizes_2206_out_o;
output   bucket_sizes_2206_out_o_ap_vld;

reg ap_idle;
reg bucket_ce0;
reg bucket_we0;
reg input_38_ce0;
reg[31:0] bucket_sizes_2213_out_o;
reg bucket_sizes_2213_out_o_ap_vld;
reg[31:0] bucket_sizes_2212_out_o;
reg bucket_sizes_2212_out_o_ap_vld;
reg[31:0] bucket_sizes_2211_out_o;
reg bucket_sizes_2211_out_o_ap_vld;
reg[31:0] bucket_sizes_2210_out_o;
reg bucket_sizes_2210_out_o_ap_vld;
reg[31:0] bucket_sizes_2209_out_o;
reg bucket_sizes_2209_out_o_ap_vld;
reg[31:0] bucket_sizes_2208_out_o;
reg bucket_sizes_2208_out_o_ap_vld;
reg[31:0] bucket_sizes_2207_out_o;
reg bucket_sizes_2207_out_o_ap_vld;
reg[31:0] bucket_sizes_2206_out_o;
reg bucket_sizes_2206_out_o_ap_vld;

(* fsm_encoding = "none" *) reg   [0:0] ap_CS_fsm;
wire    ap_CS_fsm_pp0_stage0;
wire    ap_enable_reg_pp0_iter0;
reg    ap_enable_reg_pp0_iter1;
reg    ap_idle_pp0;
wire    ap_block_state1_pp0_stage0_iter0;
wire    ap_block_state2_pp0_stage0_iter1;
wire    ap_block_pp0_stage0_subdone;
wire   [0:0] icmp_ln20_fu_223_p2;
reg    ap_condition_exit_pp0_iter0_stage0;
wire    ap_loop_exit_ready;
reg    ap_ready_int;
reg   [17:0] j_32_reg_378;
wire    ap_block_pp0_stage0_11001;
wire   [63:0] j_cast_i_fu_235_p1;
wire    ap_block_pp0_stage0;
wire   [63:0] zext_ln21_31_fu_286_p1;
wire   [31:0] bucket_sizes_fu_317_p2;
wire   [2:0] trunc_ln23_fu_291_p1;
wire    ap_loop_init;
reg   [17:0] j_fu_88;
wire   [17:0] add_ln20_fu_229_p2;
reg   [17:0] ap_sig_allocacmp_j_32;
wire   [18:0] zext_ln21_fu_277_p1;
wire   [18:0] add_ln21_fu_280_p2;
wire   [2:0] tmp_i_fu_295_p9;
wire   [31:0] tmp_i_fu_295_p10;
reg    ap_done_reg;
wire    ap_continue_int;
reg    ap_done_int;
reg   [0:0] ap_NS_fsm;
wire    ap_enable_pp0;
wire    ap_start_int;
wire    ap_ce_reg;

// power-on initialization
initial begin
#0 ap_CS_fsm = 1'd1;
#0 ap_enable_reg_pp0_iter1 = 1'b0;
#0 ap_done_reg = 1'b0;
end

multi_radix_oct_loser_64_mux_83_32_1_1 #(
    .ID( 1 ),
    .NUM_STAGE( 1 ),
    .din0_WIDTH( 32 ),
    .din1_WIDTH( 32 ),
    .din2_WIDTH( 32 ),
    .din3_WIDTH( 32 ),
    .din4_WIDTH( 32 ),
    .din5_WIDTH( 32 ),
    .din6_WIDTH( 32 ),
    .din7_WIDTH( 32 ),
    .din8_WIDTH( 3 ),
    .dout_WIDTH( 32 ))
mux_83_32_1_1_U4001(
    .din0(bucket_sizes_2206_out_i),
    .din1(bucket_sizes_2207_out_i),
    .din2(bucket_sizes_2208_out_i),
    .din3(bucket_sizes_2209_out_i),
    .din4(bucket_sizes_2210_out_i),
    .din5(bucket_sizes_2211_out_i),
    .din6(bucket_sizes_2212_out_i),
    .din7(bucket_sizes_2213_out_i),
    .din8(tmp_i_fu_295_p9),
    .dout(tmp_i_fu_295_p10)
);

multi_radix_oct_loser_64_flow_control_loop_pipe_sequential_init flow_control_loop_pipe_sequential_init_U(
    .ap_clk(ap_clk),
    .ap_rst(ap_rst),
    .ap_start(ap_start),
    .ap_ready(ap_ready),
    .ap_done(ap_done),
    .ap_start_int(ap_start_int),
    .ap_loop_init(ap_loop_init),
    .ap_ready_int(ap_ready_int),
    .ap_loop_exit_ready(ap_condition_exit_pp0_iter0_stage0),
    .ap_loop_exit_done(ap_done_int),
    .ap_continue_int(ap_continue_int),
    .ap_done_int(ap_done_int)
);

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_CS_fsm <= ap_ST_fsm_pp0_stage0;
    end else begin
        ap_CS_fsm <= ap_NS_fsm;
    end
end

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_done_reg <= 1'b0;
    end else begin
        if ((ap_continue_int == 1'b1)) begin
            ap_done_reg <= 1'b0;
        end else if (((ap_loop_exit_ready == 1'b1) & (1'b0 == ap_block_pp0_stage0_subdone) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
            ap_done_reg <= 1'b1;
        end
    end
end

always @ (posedge ap_clk) begin
    if (ap_rst == 1'b1) begin
        ap_enable_reg_pp0_iter1 <= 1'b0;
    end else begin
        if ((1'b1 == ap_condition_exit_pp0_iter0_stage0)) begin
            ap_enable_reg_pp0_iter1 <= 1'b0;
        end else if (((1'b0 == ap_block_pp0_stage0_subdone) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
            ap_enable_reg_pp0_iter1 <= ap_start_int;
        end
    end
end

always @ (posedge ap_clk) begin
    if (((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        if (((icmp_ln20_fu_223_p2 == 1'd0) & (ap_enable_reg_pp0_iter0 == 1'b1))) begin
            j_fu_88 <= add_ln20_fu_229_p2;
        end else if ((ap_loop_init == 1'b1)) begin
            j_fu_88 <= 18'd0;
        end
    end
end

always @ (posedge ap_clk) begin
    if (((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        j_32_reg_378 <= ap_sig_allocacmp_j_32;
    end
end

always @ (*) begin
    if (((icmp_ln20_fu_223_p2 == 1'd1) & (1'b0 == ap_block_pp0_stage0_subdone) & (ap_enable_reg_pp0_iter0 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ap_condition_exit_pp0_iter0_stage0 = 1'b1;
    end else begin
        ap_condition_exit_pp0_iter0_stage0 = 1'b0;
    end
end

always @ (*) begin
    if (((ap_loop_exit_ready == 1'b1) & (1'b0 == ap_block_pp0_stage0_subdone) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ap_done_int = 1'b1;
    end else begin
        ap_done_int = ap_done_reg;
    end
end

always @ (*) begin
    if (((ap_idle_pp0 == 1'b1) & (ap_start_int == 1'b0) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ap_idle = 1'b1;
    end else begin
        ap_idle = 1'b0;
    end
end

always @ (*) begin
    if (((ap_enable_reg_pp0_iter1 == 1'b0) & (ap_enable_reg_pp0_iter0 == 1'b0))) begin
        ap_idle_pp0 = 1'b1;
    end else begin
        ap_idle_pp0 = 1'b0;
    end
end

always @ (*) begin
    if (((1'b0 == ap_block_pp0_stage0_subdone) & (ap_enable_reg_pp0_iter0 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        ap_ready_int = 1'b1;
    end else begin
        ap_ready_int = 1'b0;
    end
end

always @ (*) begin
    if (((1'b0 == ap_block_pp0_stage0) & (1'b1 == ap_CS_fsm_pp0_stage0) & (ap_loop_init == 1'b1))) begin
        ap_sig_allocacmp_j_32 = 18'd0;
    end else begin
        ap_sig_allocacmp_j_32 = j_fu_88;
    end
end

always @ (*) begin
    if (((1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter1 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        bucket_ce0 = 1'b1;
    end else begin
        bucket_ce0 = 1'b0;
    end
end

always @ (*) begin
    if (((1'b0 == ap_block_pp0_stage0) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        if ((ap_loop_init == 1'b1)) begin
            bucket_sizes_2206_out_o = bucket_sizes_reload;
        end else if (((ap_enable_reg_pp0_iter1 == 1'b1) & (trunc_ln23_fu_291_p1 == 3'd0))) begin
            bucket_sizes_2206_out_o = bucket_sizes_fu_317_p2;
        end else begin
            bucket_sizes_2206_out_o = bucket_sizes_2206_out_i;
        end
    end else begin
        bucket_sizes_2206_out_o = bucket_sizes_2206_out_i;
    end
end

always @ (*) begin
    if ((((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0) & (ap_loop_init == 1'b1)) | ((1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter1 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0) & (trunc_ln23_fu_291_p1 == 3'd0)))) begin
        bucket_sizes_2206_out_o_ap_vld = 1'b1;
    end else begin
        bucket_sizes_2206_out_o_ap_vld = 1'b0;
    end
end

always @ (*) begin
    if (((1'b0 == ap_block_pp0_stage0) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        if ((ap_loop_init == 1'b1)) begin
            bucket_sizes_2207_out_o = bucket_sizes_2191_reload;
        end else if (((ap_enable_reg_pp0_iter1 == 1'b1) & (trunc_ln23_fu_291_p1 == 3'd1))) begin
            bucket_sizes_2207_out_o = bucket_sizes_fu_317_p2;
        end else begin
            bucket_sizes_2207_out_o = bucket_sizes_2207_out_i;
        end
    end else begin
        bucket_sizes_2207_out_o = bucket_sizes_2207_out_i;
    end
end

always @ (*) begin
    if ((((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0) & (ap_loop_init == 1'b1)) | ((1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter1 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0) & (trunc_ln23_fu_291_p1 == 3'd1)))) begin
        bucket_sizes_2207_out_o_ap_vld = 1'b1;
    end else begin
        bucket_sizes_2207_out_o_ap_vld = 1'b0;
    end
end

always @ (*) begin
    if (((1'b0 == ap_block_pp0_stage0) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        if ((ap_loop_init == 1'b1)) begin
            bucket_sizes_2208_out_o = bucket_sizes_2192_reload;
        end else if (((ap_enable_reg_pp0_iter1 == 1'b1) & (trunc_ln23_fu_291_p1 == 3'd2))) begin
            bucket_sizes_2208_out_o = bucket_sizes_fu_317_p2;
        end else begin
            bucket_sizes_2208_out_o = bucket_sizes_2208_out_i;
        end
    end else begin
        bucket_sizes_2208_out_o = bucket_sizes_2208_out_i;
    end
end

always @ (*) begin
    if ((((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0) & (ap_loop_init == 1'b1)) | ((1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter1 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0) & (trunc_ln23_fu_291_p1 == 3'd2)))) begin
        bucket_sizes_2208_out_o_ap_vld = 1'b1;
    end else begin
        bucket_sizes_2208_out_o_ap_vld = 1'b0;
    end
end

always @ (*) begin
    if (((1'b0 == ap_block_pp0_stage0) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        if ((ap_loop_init == 1'b1)) begin
            bucket_sizes_2209_out_o = bucket_sizes_2193_reload;
        end else if (((ap_enable_reg_pp0_iter1 == 1'b1) & (trunc_ln23_fu_291_p1 == 3'd3))) begin
            bucket_sizes_2209_out_o = bucket_sizes_fu_317_p2;
        end else begin
            bucket_sizes_2209_out_o = bucket_sizes_2209_out_i;
        end
    end else begin
        bucket_sizes_2209_out_o = bucket_sizes_2209_out_i;
    end
end

always @ (*) begin
    if ((((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0) & (ap_loop_init == 1'b1)) | ((1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter1 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0) & (trunc_ln23_fu_291_p1 == 3'd3)))) begin
        bucket_sizes_2209_out_o_ap_vld = 1'b1;
    end else begin
        bucket_sizes_2209_out_o_ap_vld = 1'b0;
    end
end

always @ (*) begin
    if (((1'b0 == ap_block_pp0_stage0) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        if ((ap_loop_init == 1'b1)) begin
            bucket_sizes_2210_out_o = bucket_sizes_2194_reload;
        end else if (((ap_enable_reg_pp0_iter1 == 1'b1) & (trunc_ln23_fu_291_p1 == 3'd4))) begin
            bucket_sizes_2210_out_o = bucket_sizes_fu_317_p2;
        end else begin
            bucket_sizes_2210_out_o = bucket_sizes_2210_out_i;
        end
    end else begin
        bucket_sizes_2210_out_o = bucket_sizes_2210_out_i;
    end
end

always @ (*) begin
    if ((((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0) & (ap_loop_init == 1'b1)) | ((1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter1 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0) & (trunc_ln23_fu_291_p1 == 3'd4)))) begin
        bucket_sizes_2210_out_o_ap_vld = 1'b1;
    end else begin
        bucket_sizes_2210_out_o_ap_vld = 1'b0;
    end
end

always @ (*) begin
    if (((1'b0 == ap_block_pp0_stage0) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        if ((ap_loop_init == 1'b1)) begin
            bucket_sizes_2211_out_o = bucket_sizes_2195_reload;
        end else if (((ap_enable_reg_pp0_iter1 == 1'b1) & (trunc_ln23_fu_291_p1 == 3'd5))) begin
            bucket_sizes_2211_out_o = bucket_sizes_fu_317_p2;
        end else begin
            bucket_sizes_2211_out_o = bucket_sizes_2211_out_i;
        end
    end else begin
        bucket_sizes_2211_out_o = bucket_sizes_2211_out_i;
    end
end

always @ (*) begin
    if ((((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0) & (ap_loop_init == 1'b1)) | ((1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter1 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0) & (trunc_ln23_fu_291_p1 == 3'd5)))) begin
        bucket_sizes_2211_out_o_ap_vld = 1'b1;
    end else begin
        bucket_sizes_2211_out_o_ap_vld = 1'b0;
    end
end

always @ (*) begin
    if (((1'b0 == ap_block_pp0_stage0) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        if ((ap_loop_init == 1'b1)) begin
            bucket_sizes_2212_out_o = bucket_sizes_2196_reload;
        end else if (((ap_enable_reg_pp0_iter1 == 1'b1) & (trunc_ln23_fu_291_p1 == 3'd6))) begin
            bucket_sizes_2212_out_o = bucket_sizes_fu_317_p2;
        end else begin
            bucket_sizes_2212_out_o = bucket_sizes_2212_out_i;
        end
    end else begin
        bucket_sizes_2212_out_o = bucket_sizes_2212_out_i;
    end
end

always @ (*) begin
    if ((((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0) & (ap_loop_init == 1'b1)) | ((1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter1 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0) & (trunc_ln23_fu_291_p1 == 3'd6)))) begin
        bucket_sizes_2212_out_o_ap_vld = 1'b1;
    end else begin
        bucket_sizes_2212_out_o_ap_vld = 1'b0;
    end
end

always @ (*) begin
    if (((1'b0 == ap_block_pp0_stage0) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        if ((ap_loop_init == 1'b1)) begin
            bucket_sizes_2213_out_o = bucket_sizes_2197_reload;
        end else if (((ap_enable_reg_pp0_iter1 == 1'b1) & (trunc_ln23_fu_291_p1 == 3'd7))) begin
            bucket_sizes_2213_out_o = bucket_sizes_fu_317_p2;
        end else begin
            bucket_sizes_2213_out_o = bucket_sizes_2213_out_i;
        end
    end else begin
        bucket_sizes_2213_out_o = bucket_sizes_2213_out_i;
    end
end

always @ (*) begin
    if ((((1'b0 == ap_block_pp0_stage0_11001) & (1'b1 == ap_CS_fsm_pp0_stage0) & (ap_loop_init == 1'b1)) | ((1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter1 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0) & (trunc_ln23_fu_291_p1 == 3'd7)))) begin
        bucket_sizes_2213_out_o_ap_vld = 1'b1;
    end else begin
        bucket_sizes_2213_out_o_ap_vld = 1'b0;
    end
end

always @ (*) begin
    if (((1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter1 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        bucket_we0 = 1'b1;
    end else begin
        bucket_we0 = 1'b0;
    end
end

always @ (*) begin
    if (((1'b0 == ap_block_pp0_stage0_11001) & (ap_enable_reg_pp0_iter0 == 1'b1) & (1'b1 == ap_CS_fsm_pp0_stage0))) begin
        input_38_ce0 = 1'b1;
    end else begin
        input_38_ce0 = 1'b0;
    end
end

always @ (*) begin
    case (ap_CS_fsm)
        ap_ST_fsm_pp0_stage0 : begin
            ap_NS_fsm = ap_ST_fsm_pp0_stage0;
        end
        default : begin
            ap_NS_fsm = 'bx;
        end
    endcase
end

assign add_ln20_fu_229_p2 = (ap_sig_allocacmp_j_32 + 18'd1);

assign add_ln21_fu_280_p2 = (zext_ln21_fu_277_p1 + 19'd156250);

assign ap_CS_fsm_pp0_stage0 = ap_CS_fsm[32'd0];

assign ap_block_pp0_stage0 = ~(1'b1 == 1'b1);

assign ap_block_pp0_stage0_11001 = ~(1'b1 == 1'b1);

assign ap_block_pp0_stage0_subdone = ~(1'b1 == 1'b1);

assign ap_block_state1_pp0_stage0_iter0 = ~(1'b1 == 1'b1);

assign ap_block_state2_pp0_stage0_iter1 = ~(1'b1 == 1'b1);

assign ap_enable_pp0 = (ap_idle_pp0 ^ 1'b1);

assign ap_enable_reg_pp0_iter0 = ap_start_int;

assign ap_loop_exit_ready = ap_condition_exit_pp0_iter0_stage0;

assign bucket_address0 = zext_ln21_31_fu_286_p1;

assign bucket_d0 = input_38_q0;

assign bucket_sizes_fu_317_p2 = (tmp_i_fu_295_p10 + 32'd1);

assign icmp_ln20_fu_223_p2 = ((ap_sig_allocacmp_j_32 == 18'd156250) ? 1'b1 : 1'b0);

assign input_38_address0 = j_cast_i_fu_235_p1;

assign j_cast_i_fu_235_p1 = ap_sig_allocacmp_j_32;

assign tmp_i_fu_295_p9 = input_38_q0[2:0];

assign trunc_ln23_fu_291_p1 = input_38_q0[2:0];

assign zext_ln21_31_fu_286_p1 = add_ln21_fu_280_p2;

assign zext_ln21_fu_277_p1 = j_32_reg_378;

endmodule //multi_radix_oct_loser_64_radix_sort_oct_batch_39_1_Pipeline_initialization
